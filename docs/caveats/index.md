...
| An empty **CP Model** field runs `cpsam`, not cyto2, and **Diameter** is a rescale factor, not a size | Your segmentation may come from a different network, at a different working scale, than the labels describe. | [Known issues](known-issues.md#6-cellpose-4-label-drift) |
...
1. **Record the Scale of every scan.** Panel 1 prints `Scale: <x> nm/px` after loading (`_widget.py:282`). For the ×4 deep-learning models an input near 100 nm/px puts the output near the 25 nm/px training domain. Put the value in your methods section, not only in your notebook.
...
9. **Keep the checkpoint filename and the architecture with the results.** Neither the CSV nor the workbook records them, and the version string reported by the Python package is wrong (see [Known issues](known-issues.md#11-the-package-reports-version-001)).
